...
\begin{center}
  \parbox{0.96\textwidth}{
    \textbf{Summary:}
    CMU M.S. ECE graduate with expertise in robot perception/localization, embedded control, bio-inspired locomotion, and autonomous planning. Currently developing \textbf{LiDAR-inertial SLAM/localization} and parallelized \textbf{C++ IK and motion planning pipelines} for wheeled humanoid warehouse robots at Yondu AI. Previously embedded software engineer at Cepton (LiDAR systems) and research assistant in the CMU Biorobotics Lab (\textbf{EigenBot}, \textbf{MMPUG}). Published in ICRA 2025, AAAI 2023, and IEEE Neural Networks. Seeking research collaboration in Embodied AI systems, world models for robotics, real-time robot learning, and scalable deployment of foundation models on physical platforms.
  }
\end{center}
\vspace{-1.5em}
...
\resumeResearchMidItem{}
{Ouster Integration, Dynamic IMU Estimation, and LiDAR-Inertial SLAM}
{
  \resumeResearchItemListStart
  \resumeResearchItem{}
  {Resolved persistent \textbf{asymmetric yaw drift and odometry snapping} for Ouster odometry through \textbf{extrinsic calibration} (KISS-ICP), EKF fusion tuning, \textbf{dynamic IMU bias correction}, and \textbf{point cloud deskewing}.}
  \resumeResearchItem{}
  {Reproduced \textbf{Fast-LIO2} and \textbf{LIO-SAM} across Livox, Ouster, Velodyne, and internal Ouster datasets, generating clean PCD maps and implementing \textbf{Fast-LIO2 localization} for Ouster-based robot operation beyond prior 2D SLAM Toolbox workflows.}
  \resumeResearchItemListEnd
}
\resumeResearchMidItem{}
{Data Collection and Post-processing Pipeline Optimization \& Acceleration}
{
  ...
  {Redesigned cloud-to-dataset post-processing pipeline, achieving \textbf{5-7x speedup} and improving throughput from \textbf{28 to 145 FPS}; reduced cloud transfer by \textbf{92\%} via parallel segment download and coverage-aware data selection.}
  \resumeResearchItemListEnd
}
\resumeResearchMidItem{}
{Warehouse Robot Field Deployment and Patrol System}
{
  \resumeResearchItemListStart
  \resumeResearchItem{}
  {Deployed robots at \textbf{GPA Logistics} and \textbf{Cainiao} warehouses, handling site network configuration, production bring-up, field debugging, and GPA patrol replay/coverage benchmarking.}
  \resumeResearchItemListEnd
}
...
\vspace{-5pt}
\resumeResearchItem{}
{Studied unsupervised graph representation learning and robustness by analyzing GAT/GCN behavior within Deep Graph Infomax (DGI), integrating mutual information maximization, and benchmarking adversarial defenses against Metattack and Nettack, including RoSA, Pro-GNN, PA-GNN, GraphCL, and GraphSS.}
\resumeResearchItem{}
...
\resumeProjectsItem{Implemented DBMS storage layer (LRU-K, disk scheduler, buffer   
